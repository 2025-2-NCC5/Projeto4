...
def create_customers(cur, n_cust=2000):
    ...
    for _ in range(n_cust):
        cid = str(uuid.uuid4())
        ids.append(cid)
        lat = round(random.uniform(-23.7, -22.9), 6)
...
def create_campaigns(cur, n_campaigns=20):
    types = ["desconto", "cashback", "combo"]
    segments = ["VIP", "Ocasional", "Inativo"]
    ids = []
    for _ in range(n_campaigns):
        cid = str(uuid.uuid4())
        ids.append(cid)
        name = f"Campanha {fake.word().capitalize()}"
        ...
        cur.execute(
            "INSERT INTO campaigns(id, name, type, target_segment, expected_roi, expected_revenue, expected_engagement) VALUES (%s,%s,%s,%s,%s,%s,%s)",
            (cid, name, ctype, segment, roi, revenue, engagement)
        )
    return ids

def create_campaign_interactions(cur, campaigns, customers, n_interactions=5000):
    for _ in range(n_interactions):
        iid = str(uuid.uuid4())
        campaign_id = random.choice(campaigns)
        customer_id = random.choice(customers)
        interaction_type = random.choices(["visualizou", "clicou", "comprou"], weights=[0.5, 0.3, 0.2])[0]
        interacted_at = datetime.utcnow() - timedelta(days=random.randint(0,90))
        cur.execute(
            "INSERT INTO campaign_interactions(id, campaign_id, customer_id, interaction_type, interacted_at) VALUES (%s,%s,%s,%s,%s)",
            (iid, campaign_id, customer_id, interaction_type, interacted_at)
        )

def create_reviews(cur, restaurants, customers, n_reviews=3000):
    sentiments = [-1, -0.5, 0, 0.5, 1]
    texts = [
        "Comida excelente e entrega rápida!",
        "Demorou demais, não gostei.",
        "Bom custo-benefício, voltarei a pedir.",
        "Atendimento ruim, mas comida boa.",
    ...
    for _ in range(n_reviews):
        ...
        cur.execute(
            "INSERT INTO reviews(id, customer_id, restaurant_id, review_text, sentiment_score, created_at) VALUES (%s,%s,%s,%s,%s,%s)",
            (rid, customer_id, restaurant_id, review_text, sentiment_score, created_at)
        )
...
def main():
    ...
    campaigns = create_campaigns(cur, n_campaigns=20)
    print("Creating campaign interactions...")
    create_campaign_interactions(cur, campaigns, customers, n_interactions=5000)
    print("Creating reviews...")
    create_reviews(cur, restaurants, customers, n_reviews=3000)
